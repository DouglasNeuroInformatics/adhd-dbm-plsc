...
if (!file.exists(ANAT_FILE)) {
  ...
  cat(sprintf("Cropped dim1: %d-%d, dim2: %d-%d, dim3: %d-%d\n",
              dim1_begin, dim1_end, dim2_begin, dim2_end, dim3_begin, dim3_end))
  ...
  for (lv in sig_lvs) {
    ...
    tryCatch({
      sliceSeries(nrow = 5, ncol = 2, dimension = 2, begin = dim2_begin, end = dim2_end) %>%
        ...
        addtitle("Coronal") %>%
        overlay(bsr_cropped, low = 0, high = upperthreshold, symmetric = TRUE,
    ...
    # Unthresholded
    fname_unthresh <- file.path(OUTPUT_DIR, sprintf("plot_D_brainmap_LV%d_unthresholded.pdf", lv))
    pdf(fname_unthresh, width = 12, height = 8)
    tryCatch({
      sliceSeries(nrow = 5, ncol = 2, dimension = 2, begin = dim2_begin, end = dim2_end) %>%
        anatomy(anat_cropped, low = 0.5, high = 9.7,
          col = colorRampPalette(c("black", "grey50", "white"))(255)) %>%
        addtitle("Coronal") %>%
        overlay(bsr_cropped, low = round(BSR_THRESH, 1), high = upperthreshold, symmetric = TRUE,
                col = colorRampPalette(c("red", "yellow", "white"))(255),
                rCol = colorRampPalette(c("blue", "turquoise1", "white"))(255)) %>%
      sliceSeries(nrow = 5, ncol = 2, dimension = 1, begin = dim1_begin, end = dim1_end) %>%
        anatomy(anat_cropped, low = 0.5, high = 9.7,
          col = colorRampPalette(c("black", "grey50", "white"))(255)) %>%
        addtitle("Sagittal") %>%
